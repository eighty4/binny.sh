...
import SystemLogo from '../../../../libs/components/src/SystemLogo.ts'
...
export default class RepositoryLink extends HTMLElement {
    ...
    #render() {
        this.#shadow
            ...
            .setAttribute('owner', this.#repo.owner)
        if (this.#repo.script) {
            this.#shadow
                .querySelector('.link')!
                .appendChild(
                    this.#createScriptVersion(
                        this.#repo.script.templateVersion,
                    ),
                )
        }
        this.#shadow.querySelector('.name')!.textContent =
            `${this.#repo.owner}/${this.#repo.name}`
        this.#shadow.querySelector('.update')!.textContent =
        ...
        if (this.#repo.latestRelease) {
            ...
            this.#shadow!.querySelector('.os-logos')!.append(
                ...oses.map(os => new SystemLogo({ os })),
            )
        }
    }
    ...
    #createScriptVersion(templateVersion: string): HTMLElement {
        ...
        scriptVersion.appendChild(
            document.createTextNode(templateVersion + ' is up to date'),
        )

...
export default class DownloadPanel extends HTMLElement {
    ...
    constructor() {
        ...
        this.#downloadLinuxButton = this.#shadow.querySelector(
            'button[data-os="linux"]',
        )!
        this.#downloadWindowsButton = this.#shadow.querySelector(
    ...
    connectedCallback() {
        ...
        this.#downloadWindowsButton.addEventListener(
            'click',
    ...
    disconnectedCallback() {
        this.#downloadLinuxButton.removeEventListener(
            'click',
            this.#downloadLinux,
        )
        this.#downloadWindowsButton.removeEventListener(
            'click',
            this.#downloadWindows,
        )
    ...
    update(configuration: ScriptConfiguration) {
        this.#downloadLinuxButton.disabled = !(
            configuration.areOsBinariesResolved('Linux') &&
            configuration.areOsBinariesResolved('MacOS')
        )
        this.#downloadWindowsButton.disabled =
            !configuration.areOsBinariesResolved('Windows')
    }
    ...
    #download(detail: OperatingSystem) {
        this.dispatchEvent(
            new CustomEvent<OperatingSystem>(DOWNLOAD_SCRIPT_EVENT_TYPE, {
                detail,
            }),
        )
    }

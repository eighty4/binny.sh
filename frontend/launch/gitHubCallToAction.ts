...
const getRandomBugPath = () =>
    `path("${bugPaths[Math.floor(Math.random() * bugPaths.length)]}")`

export function initializeGitHubCallToAction() {
    const githubContainer = document.getElementById('github') as HTMLElement
    let callToActions = githubContainer.querySelectorAll(
        '.call-to-action',
    ...
    setInterval(() => {
        ...
        const zombieBug = githubContainer.querySelector(
            '#bug',
        ) as HTMLElement | null
        if (zombieBug) {
